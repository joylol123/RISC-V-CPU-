...
always_ff @(posedge clk or posedge rst) begin
    ...
    else if(Dy_B_branch_flag==1'b1 && pc!=pc_reg)begin
        BB_counter <= BB_counter + 2'd1;
        pc_reg <= pc;
    end
    else begin
        BB_counter <= 2'd0;
...
always_ff @(posedge clk or posedge rst) begin
    if(rst) begin
        Dy_B_branch_flag_reg <= 1'b0;
        branch_sel_Dy_reg <= 2'b0;
    end
...
// ========
always_comb begin
    if(Dy_B_branch_flag == 1'b1 && Dy_B_branch_flag_reg == 1'b1)
        correct_pc_BB_flag = 1'b1;
    else 
        correct_pc_BB_flag = 1'b0;
end
...
always_comb begin // When btype run to exe stage chose the right pc
    if(branch_sel_Dy_reg[1] ==1'b1 && true_branch == 1'b0)begin
        correct_pc = correct_pc_4; 
   
    end
    else if(branch_sel_Dy_reg[1] == 1'b0 && true_branch == 1'b1)begin
...
always_comb begin
    if( Dy_B_branch_flag_reg== 1'b1  )begin
        case (curr_state)
            STRONG_TAKEN:begin
                if( true_branch==1'b1 && branch_sel_Dy_reg==2'b01)begin
            ...
            WEAK_NOT_TAKEN:begin
                if( true_branch==1'b1&&branch_sel_Dy_reg==2'b00)begin
...
always_comb begin
    ...
    // endcase 
        if(branch_sel_Dy_t_nt_reg == 2'b01 && true_branch == 1'b0) B_branch_count = 2'd2; // jump but not jump
        else if(branch_sel_Dy_t_nt_reg == 2'b01 && true_branch == 1'b1) B_branch_count = 2'd2;//jump and jump
        else if(branch_sel_Dy_t_nt_reg == 2'b00 && true_branch == 1'b0) B_branch_count = 2'd2;// not jump and not jump
        else if(branch_sel_Dy_t_nt_reg == 2'b00 && true_branch == 1'b1) B_branch_count = 2'd2;//not jump but jump
...
always_comb begin // B_branch
    if(Dy_B_branch_flag == 1'b1  )begin //&& branch_sel_Dy_reg != 2'd2
        case (curr_state)
            STRONG_TAKEN:begin
        ...
        if(branch_sel_Dy_reg ==2'd1 && true_branch == 1'b0)begin
            branch_sel_Dy = correct_pc_sel;
        end
        else if(branch_sel_Dy_reg == 2'd0 && true_branch == 1'b1)begin
            branch_sel_Dy = correct_pc_sel;
        end
        else begin
